    ...
    .controller('HomeCtrl', function HomeController($scope, $http, store, $state, apiUrl) {
        ...
        $scope.userGrid = {
                ...
                schema: {
                    ...
                    data: function (response) {
                        return response.map(function (item) {
                            if (item.authorities.some(authority => authority.authority === 'ADMIN'))
                                roles = 'ADMIN';
                            else
                                roles = item.authorities.map(auth => auth.authority.replace('', '')).join(', ');
                            return {
                                image: item.image,
                                id: item.id,
            ...
            columns: [
                ...
                <button kendo-button ng-click="getUserHistory(dataItem)">
        ...
        $scope.getUserHistory = function (dataItem) {
            $http({
                method: 'GET',
                url: apiUrl + '/documents/' + dataItem.id,
                headers: { 'Authorization': "Bearer: " + JSON.parse(localStorage.getItem("jwt")) }
            })
                .then(function (response) {
                    $scope.userHistory = response.data;
                    $scope.userHistoryWindow.center().open();
